...
enrollments |> 
  group_by(dept) |> 
  summarize(avg_enroll = mean(enroll, na.rm = TRUE)) |> 
  arrange(desc(avg_enroll))

enrollments |> 
  group_by(dept) |> 
  filter(n() >= 5) |> 
  ggplot(aes(x = reorder(dept, enroll, FUN = median), y = enroll)) +
  geom_boxplot() +
  coord_flip() +
  labs(title = "Enrollment Distribution by Department",
       x = "Department", y = "Enrollment")
...
```{r}
#top instructors by total enrollments
enrollments_2 |> 
  group_by(instructor) |> 
  summarize(total_enroll = sum(enroll, na.rm = TRUE)) |> 
...
enrollments_2 |> 
  ...
  geom_col() +
  coord_flip() +
  labs(title = "Top 10 Instructors by Total Enrollment",
       x = "Instructor", y = "Total Enrollment")
```
