...
```{r}
# What does this do? The code filter out the data and give data only on Delaware and Hawaii
elections_small |>
...
```{r}
# What did this code do? create a column called diff_20 that subtract dem_pct_20 from repub_pct_20
elections_small |>
...
```{r}
# What did this code do? create a new column that perform calculations and round up the numbers to whole number
elections_small |>
...
```{r}
# What did this code do? create a binary variable and show true if repub_pct_20 > dem_pct_20
elections_small |>
...
elections_small |>
 mutate(dem_year_difference = dem_pct_20 - dem_pct_16) |>
 head()
```
...
elections_small |>
 mutate(dem_2020higher = dem_pct_20 > dem_pct_16) |>
 head()
```
...
BEFORE running the below chunk, what do you think it will produce? the same result. 
...
So the order of `filter()` and `arrange()` did *not* matter -- rerranging them produces the same results. BUT what is one *advantage* of filtering before arranging? After filtered out the unwanted values, the dataset is cleaner. 
...
BEFORE trying, what do you think will happen if you change the order of mutate and select:1
...
The demoncrate support shifted from 2016-2020. 
```{r}
...
```{r}
# What does this do? calculate the median of repub_pct_20
elections_small |>
  summarize(median(repub_pct_20))
```

```{r}
# What does this do?calculate the median of median_repub
elections_small |>
  summarize(median_repub = median(repub_pct_20))
```

```{r}
# What does this do?calculate the median and the total votes 
elections_small |>
...
```{r}
# What does this do? calculate the median 2020 Repub percent and total votes by state
...
elections_small |>
  ...
  summarize(dem_votes = sum(dem_pct_20),
            rep_votes = sum(repub_pct_20),
            .groups = "drop") |>
  group_by(state_name) |>
  summarize(dem_total = sum(dem_votes),
            rep_total = sum(rep_votes))
...
world_cup |>
  summarize(median(goals_scored), min(goals_scored), max(goals_scored))
