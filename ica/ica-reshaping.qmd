...
The units of observation is the subjects in the study. 
...
The will be three columns, subject, number of days, reaction time. 
...
sleep_wide |>
  pivot_longer(cols = starts_with("day"), names_to = "day", values_to = "reaction time")
```
...
-   Besides putting each argument on a different line for readability and storing the results, what changed in the code? It adds the  "names_prefix = "day_"
-   How did this impact how the values are recorded in the `day` column?
the values turn in to integers
...
Reaction time increase if the individuals lack sleep time. It's not the same for everyone, some subjects are impacted more and some are les.s 
